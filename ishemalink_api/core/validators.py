...
def validate_rwanda_nid(nid: str) -> Tuple[bool, str | None]:
    ...
    try:
        ...
        if year < 1900 or year > 2010:
            return False, f"Invalid birth year: {year}. Must be between 1900 and 2010"
    except ValueError:
        return False, "Invalid year format in NID"
    
    # Validate province code (position 5, should be 1-5)
    province_code = int(nid[5])
    if province_code < 1 or province_code > 7:  # Extended for edge cases
